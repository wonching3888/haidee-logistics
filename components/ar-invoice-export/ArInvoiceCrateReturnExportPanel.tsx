...
import {
  exportArCrateReturnCsvAction,
  getArCrateReturnExportPreview,
} from "@/app/actions/ar-invoice-export";
import { cn } from "@/lib/utils";

type PreviewData = Awaited<ReturnType<typeof getArCrateReturnExportPreview>>;

function money(value: number) {
  return value.toFixed(2);
}

export function ArInvoiceCrateReturnExportPanel() {
  const now = new Date();
  ...
  function loadPreview() {
    startTransition(async () => {
      setError(null);
      try {
        const result = await getArCrateReturnExportPreview({ year, month });
        setPreview(result);
      } catch (e) {
        setPreview(null);
  ...
  function handleExport() {
    startTransition(async () => {
      setError(null);
      try {
        const result = await exportArCrateReturnCsvAction({ year, month });
  ...
  const printHref = `/documents/ar-invoice-listing/crate-return/print?year=${year}&month=${month}&returnTo=${encodeURIComponent("/financial/autocount-export")}`;

  return (
    <section className="space-y-4 rounded-xl border border-haidee-border bg-white p-4">
      <div>
        <h3 className="font-semibold">回桶费导出 Crate Return AR Export</h3>
        <p className="mt-1 text-sm text-haidee-muted">
          按月份导出回桶月结单 AR CSV（金额与收账 / 打印页同源；描述为「收桶费」）。
        </p>
      </div>
      ...
      {preview && preview.skipped.length > 0 ? (
        <div className="rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
          <p className="font-medium">已跳过 Skipped ({preview.skipped.length})</p>
          <ul className="mt-1 list-inside list-disc">
            {preview.skipped.map((item) => (
              <li key={item.entityKey}>
                {item.debtorName}
                {item.debtorCode ? ` (${item.debtorCode})` : ""}: {item.reason}
              </li>
            ))}
          </ul>
        </div>
      ...
      {preview && preview.rows.length > 0 ? (
        ...
        <p className="text-sm text-haidee-muted">该月无可导出的回桶费 Invoice。</p>
      ) : null}
