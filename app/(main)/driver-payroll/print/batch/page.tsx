...
import { getBatchDriverPayslipPrintData } from "@/app/actions/driver-payroll";
...
interface DriverPayslipBatchPrintPageProps {
...
export default async function DriverPayslipBatchPrintPage({
  searchParams,
}: DriverPayslipBatchPrintPageProps) {
  ...
  try {
    const data = await getBatchDriverPayslipPrintData({ year, month });
    if (data.entries.length === 0) notFound();

    const documentTitle = `Payslip-Batch-${data.yearMonth}`;
    const backHref =
      returnTo ||
      `/driver-payroll?year=${year}&month=${month}&tab=summary`;
    ...
    return (
      <DOPrintPageWithShare
        ...
        sharePayload={{
          ...
          title: documentTitle,
          text: `Batch payslip ${data.yearMonth} · ${data.entries.length} drivers`,
        }}
        sectionSelector=".driver-payslip-batch-page"
      >
        <DriverPayslipBatchPrint
          year={data.year}
          month={data.month}
